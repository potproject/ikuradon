        ...
        return (
            <ScrollView>
                <View style={styles.container}>
                    <Button style={styles.button} onPress={() => this.props.onCancel()} title={I18n.t("global_back")} />
                    {this.state.emojis.map(emoji => {
                        if (typeof emoji.visible_in_picker !== "undefined" && !emoji.visible_in_picker) {
                            return null;
                        }
                        return (
                            <View key={emoji.shortcode} style={styles.emojiList}>
                                <TouchableOpacity
                                    onPress={() => {
                                        this.send(emoji.shortcode);
                                    }}
                                >
                                    <Image style={styles.emoji} source={{ uri: emoji.url }} />
                                </TouchableOpacity>
                            </View>
                        );
                    })}
